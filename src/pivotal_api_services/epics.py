...
class EpicServices(PivotalServices):
    ...
    def get_epics(self, id_project):
        current_url = self.endPoint.format(id_project)
        epics_list = self.request_handler.get_request(endpoint=current_url).json()
    ...
    def delete_all_epics(self, id_project):
        self.get_epics(id_project)
        for epic in self.epics.values():
            url = self.endPoint.format(id_project) + "/" + str(epic)
